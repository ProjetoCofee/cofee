...
class Estoque_Controller extends Controller
{
	public function index($atributo){
		...
		if($atributo == "show"){
			...
			$produtos = DB::table('produtos')->join(
				'marcas', 'produtos.id_marca', '=', 'marcas.id')->join(
					'departamentos', 'produtos.id_departamento', '=', 'departamentos.id')->select(
						'produtos.id', 
						...
						'produtos.prateleira', 
						'produtos.minimo', 
						'produtos.observacao')->orderByRaw('descricao ASC')->paginate(7);

					return view('estoque.estoque_show',compact('produtos'));
				}else if($atributo == "historico_entrada"){

					$entradas = DB::table('entrada')->join(
						'users', 'entrada.id_usuario', '=', 'users.id')->select(
							'entrada.id',
							'entrada.id_usuario',
							'entrada.data_entrada',
							'entrada.motivo',
							'users.name as responsavel')->orderByRaw('data_entrada DESC')->paginate(7);
						...
						$solicitacoes = DB::table('solicitacao_produto')->select(
							'solicitacao_produto.id',
							'solicitacao_produto.id_usuario_solicitante as solicitante',
							'solicitacao_produto.id_usuario_aprova as aprovador',
							'solicitacao_produto.data_solicitacao',
							'solicitacao_produto.data_aprovacao',
							'solicitacao_produto.status')->orderByRaw('data_solicitacao DESC')->paginate(7);
						...
						$solicitacoes = DB::table('solicitacao_compra')->join(
							'users', 'solicitacao_compra.id_usuario_solicitante', '=', 'users.id')->join(
								'produtos', 'solicitacao_compra.id_produto', '=', 'produtos.id')->select(
									...
									'solicitacao_compra.confirmado')->orderByRaw('data_solicitacao DESC')->paginate(7);
